...
describe('PersonModel', function() {
  ...
  describe('#create()', function() {
    it('should create a new Person', function (done) {
      Person.create({first_name: 'Apple', last_name:'Banana'})
  ...
  describe('#update({user:"someUser"})', function() {
    it('should associate a User to the Person', function (done) {
      ...
      User.create({email: 'associateUser@email.com', password:'password123'})
      .then(function(user_res) {
        // some tests
        Person.update({first_name:'Apple'},{user:user_res}).exec(function(err, res){
          Person.findOne({first_name:'Apple'}).populate('user').exec(function(err1, res1){
